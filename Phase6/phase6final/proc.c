...
void TermProc()
{
   	...
   	while(1) 
	{
		...
		TermOut("\t*** Welcome! Commands are: ***\n\tls [file], cat<file>, logout\n\t(can also use 11/222/000000 instead)\n",local);
		while(1)
		{
			...
			{
				TermOut("Stop fooling around, get to work!\n",local);
			}
		}
	}
}
...
{ // make str from attributes in str
	...
	char *name = (char *)(attr_p + 1);
   	...
   	// make str from the attributes attr_p points
      	...
      	if ( A_ISDIR(attr_p->mode) ) 
		str[1] = 'd';         // mode is directory
      	if ( QBIT_ON(attr_p->mode, A_ROTH) ) 
		str[2] = 'r'; // mode is readable
      	if ( QBIT_ON(attr_p->mode, A_WOTH) ) 
		str[3] = 'w'; // mode is writable
      	if ( QBIT_ON(attr_p->mode, A_XOTH) ) 
		str[4] = 'x'; // mode is executable
}
