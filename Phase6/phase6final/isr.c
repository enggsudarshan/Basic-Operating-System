...
void FcloseISR(void) 
{
   	...
   	if( result == 0 ) 
		...
		cons_printf( "FcloseISR: cannot close FD!\n" );
}
...
// start our search there
   	if( name[0] == '/' ) 
	{
			...
			name++;
      		...
      		if( name[0] == 0 ) 
			return root_dir; // client asked for "/"
   	} 
	else 
	{
...
dir_t *FindNameSub( char *name, dir_t *this_dir ) 
{
	...
	// if name is '.../...,' we decend into subdir
   	if( ( p = strchr( name, '/' ) ) != 0) 
		len = p - name;  // p = to where / is (high mem)
	...
	//      if((unsigned int)dir_p->name > 0xdfffff) return 0; // tmp bug-fix patch
		...
		{
         		if( p && p[1] != 0 ) 
			{ // not ending with name, it's "name/..."
			  ...
			  // is a dir. if name ends with "/" we don't check. thus "hello.html/" is legal
            			while( *p == '/' ) 
				{
               				...
               				if( '\0' == *p ) 
						return dir_p; // name "xxx/////" is actually legal
            			}

				// altho name given is "xxx/yyy," xxx is not a directory
            			if(dir_p->mode != MODE_DIR) 
					return 0; // bug-fix patch for "cat h/in"
