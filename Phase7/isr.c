...
void ForkISR(void) 
{
	...
	MyMemcpy((char *)page_info[page_num].addr, (char *)pcb[run_pid].TF_p->eax, pcb[run_pid].TF_p->ebx);
	
	MyBzero((char *)&pcb[pid], sizeof(pcb_t));
 

	pcb[pid].TF_p = (TF_t *) (page_info[page_num].addr + 4096-(sizeof(TF_t)));

   	pcb[pid].TF_p->eip=(unsigned int) page_info[page_num].addr + 128;
