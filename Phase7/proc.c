...
void TermProc()
{
   	...
   	char login[101], passwd[101], cmd_str[101],obj_data[101];
	int exit_status, cpid;
   	...
   	while(1) 
	{
		...
		while(1)
		{
			...
			Fstat(cmd_str,obj_data);
				...
				{
					Fork(attr_p->data,attr_p->size);
					cpid = Wait(&exit_status);
					cons_printf("Child PID %d, exits status %x.\n", cpid, exit_status);
