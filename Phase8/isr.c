...
void NewProcISR(int new_pid, func_ptr_t p) 
{
	...
	pcb[new_pid].runtime=pcb[new_pid].lifespan=0;
        pcb[new_pid].main_table = kernel_main_table;
     	if(new_pid > 0)        
       		EnQ(new_pid,&ready_q);
...
void ForkISR(void) 
{
	...
	{
		if (page_info[i].owner == -1)
                { 
			five_num[page_num++] = i;
                   	if(page_num == 5) //if git enough indexes break
			{
				break;
			}
	...
	if(page_num != 5 )
	{
		cons_printf("Kernel Panic: not getting five DRAM pages!\n");
         	return;
	}
	...
	if(pid == -1)
        ...
        for (i = 0; i < 5; i++)
	{
		page_info[five_num[i]].owner = pid;
		MyBzero((char *)page_info[five_num[i]].addr,4096);//use PROC_STACK_SIZE
	}


	main_table = page_info[five_num[0]].addr;
	codedata_table = page_info[five_num[1]].addr;
	stack_table = page_info[five_num[2]].addr;
	codedata_page= page_info[five_num[3]].addr;
	stack_page = page_info[five_num[4]].addr;

	MyMemcpy((char *)codedata_page, (char *)code_addr,code_size);
	...
	pcb[pid].main_table =main_table;

	pcb[pid].TF_p = (TF_t *)(0xffffffff - sizeof(TF_t) + 1);


	TF_p =(TF_t *)(stack_page + PAGE_SIZE - sizeof(TF_t));

	TF_p->eip = (unsigned int)(0x80000080);
	...
	MyMemcpy((char*)main_table,(char*)kernel_main_table,16);

         _p = (int*)(main_table);
	_p[512] = codedata_table + 0x003;
	_p[1023] = stack_table + 0x003;

	_p=(int*)(codedata_table);
	_p[0] = codedata_page + 0x003;


	_p=(int*)(stack_table);
	_p[1023] = stack_page +0x003;   
...
void WaitISR(void) 
{
	...
	int j, i, childProc,*exit_status, child_exit_status;
	childProc = 20;

	for (j = 0; j < PROC_NUM; j++)
	{
		if (  (pcb[j].ppid == run_pid)&& (pcb[j].state == ZOMBIE))
		{
			childProc = j;
			break;
		}
	...
	exit_status = (int *)pcb[run_pid].TF_p->eax;
        child_exit_status = pcb[childProc].TF_p->eax;
        pcb[run_pid].TF_p->ebx = childProc;
        pcb[ppid].TF_p->eax=run_pid;
        *exit_status = child_exit_status;
              
	for(i = 0; i < PAGE_NUM; i++)
	{
		if (page_info[i].owner == childProc)
		{
			page_info[i].owner = -1;
			MyBzero((char *)page_info[i].addr,4096);
			break;
		}
	}

        pcb[childProc].state= AVAIL;
	EnQ(childProc, &avail_q);
	MyBzero((char *)&pcb[childProc], sizeof(pcb_t));
...
void ExitISR(void) 
{
	int ppid, i, *exit_p, child_exit;
	ppid = pcb[run_pid].ppid;
	if (pcb[ppid].state != WAIT2CHILD)
	{
		...
		return;
	        ...
	        child_exit = pcb[run_pid].TF_p->eax;
		exit_p = (int *) pcb[ppid].TF_p->eax;
...
void SysWriteISR(void)
{
	...
	MyStrcpy(term[which].stdout_q,(char *)pcb[run_pid].TF_p->eax);
